...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| Python 与依赖说明 | 完成 | README「环境要求」：`Python 3.10+`，`pip install -r requirements.txt` |
| 示例配置无密钥 | 完成 | `config/app_runtime.example.json` 中 `api_key` 为空字符串 |
| 本地配置不入库 | 完成 | `.gitignore` 含 `config/app_runtime.json`；另忽略 `.env`、`.env.*`（保留可选 `!.env.example`） |
| 术语固定 | 完成 | [`docs/glossary.md`](glossary.md) |
| 配置目录说明 | 完成 | [`config/README.md`](../config/README.md) |
| 快速开始与示例模型 | 完成 | README 与 `app_runtime.example.json` 均指向 Ollama 示例 `ollama/qwen3:8b` / `qwen3:8b` |
...
- **运行时字段**：`soul_package_enabled`（JSON 布尔）、`soul_package_path`（字符串）。相对路径相对于**项目根目录**。
- **服务端**：`src/server.py` 在关闭、路径空、或 `try_load_soul_package` 失败时，**仅**使用原 `system_prompt`（不注入金规则）；成功时顺序为：金规则 → `system_prompts.txt` → profile 紧凑摘要 → `system_prompt`。
- **设置页**：`/settings` 提供开关与路径输入，经 `POST /api/runtime-config` 写入 `app_runtime.json`。
